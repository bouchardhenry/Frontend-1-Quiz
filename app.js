...
function startQuiz(categoryId, sectionToHide) {
  ...
  fetch(
    `https://opentdb.com/api.php?amount=5&category=${categoryId}&difficulty=medium&type=multiple`
  )
    .then((res) => res.json())
    .then((data) => {
...
function renderQuestion() {
  ...
  buttons.forEach((btn, i) => {
    ...
    btn.onclick = () =>
      handleAnswer(text, correctAnswer, text === correctAnswer);
  });
}

// Funktion som körs när användaren väljer ett svar
function handleAnswer(selectedAnswer, correctAnswer, isCorrect) {
  scoreLog.push({
    // Sparar frågan, valt svar och om det var rätt eller fel
...
Category1Btn.addEventListener("click", () => {
  ...
  category1Text.textContent =
    "Test your knowledge of the world with our geography quiz! From capitals to landmarks, challenge yourself and discover fun facts along the way.";
...
Category2Btn.addEventListener("click", () => {
  ...
  category2Text.textContent =
    "Dive into myths and legends from around the world. Test your knowledge of gods, heroes, and mythical creatures!";
  show(category2Section);
});

// Starta quizen för vald kategori
fetchGeoQuestions.addEventListener("click", () =>
  startQuiz(22, category1Section)
);
fetchMythologyQuestions.addEventListener("click", () =>
  startQuiz(20, category2Section)
);
